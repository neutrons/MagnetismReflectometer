...
- Update pixi environment ``mr_reduction`` or ``mr_reduction_qa`` by deploying a new version.
  If we are testing a new feature, we'll be deploying the pixi environment ``mr_reduction_qa``.
...
.. code-block:: bash
   ...
   $ cp /usr/local/pixi/mr_reduction_qa/.pixi/envs/default/lib/python3.11/site-packages/mr_livereduce/reduce_REF_M_live_post_proc.py /SNS/REF_M/shared/livereduce/
...
.. code-block:: bash
   ...
   $ nsd-pixi-shell.sh mr_reduction
